...
// === 盈利再投资 ===
async function loadProfitRecyclingStatus() {
  const wheelCard = document.getElementById('sc-wheel');
  if (!wheelCard) return;
  
  // 移除旧的进度条
  const oldBar = wheelCard.querySelector('.recycle-bar');
  if (oldBar) oldBar.remove();
  
  try {
    const res = await fetch('/api/profit-recycling/status');
    const data = await res.json();
    if (!data.enabled) return;
    
    const inner = wheelCard.querySelector('.strategy-body-inner');
    if (!inner) return;
    ...
    const barHtml = `
      <div class="recycle-bar">
        <div class="recycle-header">
          <span>♻️ 盈利再投资</span>
          <span class="recycle-info">已循环: $${data.total_recycled.toLocaleString()} | 阈值: $${data.threshold.toLocaleString()}</span>
        </div>
        <div class="recycle-progress">
          <div class="recycle-progress-track">
            <div class="recycle-progress-fill" style="width: ${data.progress_pct}%"></div>
          </div>
          <span class="recycle-progress-text">$${Math.round(data.current_cycle_profit).toLocaleString()} / $${data.threshold.toLocaleString()}</span>
        </div>
        ${data.can_recycle ? `<button class="btn btn-sm btn-recycle" onclick="showRecycleModal()">♻️ 再投资 $${data.recycle_amount.toLocaleString()} 到定投仓</button>` : ''}
      </div>
    `;
    
    inner.insertAdjacentHTML('afterbegin', barHtml);
  } catch(e) {
    console.error('加载盈利再投资状态失败', e);
  }
}

async function showRecycleModal() {
  // 获取 DCA 持仓列表
  const dcaPositions = (portfolio.positions || []).filter(p => p.strategy === 'dca');
  let optionsHtml = dcaPositions.map(p => 
    `<option value="${p.id}">${p.symbol} (${p.quantity}股, 均价$${p.avg_price.toFixed(2)})</option>`
  ).join('');
  
  const modalHtml = `
    <div class="modal active" id="recycle-modal">
      <div class="modal-backdrop" onclick="closeRecycleModal()"></div>
      <div class="modal-content" style="max-width: 480px">
        <div class="modal-header">
          <h3>♻️ 盈利再投资</h3>
          <button class="modal-close" onclick="closeRecycleModal()">×</button>
        </div>
        <div class="modal-body">
          <p>选择目标定投持仓（或新建）：</p>
          <div style="margin: 12px 0">
            <label><input type="radio" name="recycle-target" value="existing" checked onchange="toggleRecycleTarget()"> 已有持仓</label>
            <label style="margin-left:16px"><input type="radio" name="recycle-target" value="new" onchange="toggleRecycleTarget()"> 新建持仓</label>
          </div>
          <div id="recycle-existing">
            <select id="recycle-pos" class="form-input" style="width:100%">
              ${optionsHtml || '<option value="">暂无DCA持仓</option>'}
            </select>
          </div>
          <div id="recycle-new" style="display:none; margin-top:12px">
            <input id="recycle-symbol" class="form-input" placeholder="代码 如 AAPL" style="margin-bottom:8px; width:100%">
            <input id="recycle-qty" class="form-input" type="number" placeholder="数量" style="margin-bottom:8px; width:100%">
            <input id="recycle-price" class="form-input" type="number" step="0.01" placeholder="买入价" style="width:100%">
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="closeRecycleModal()">取消</button>
          <button class="btn btn-primary" onclick="executeRecycle()">确认再投资</button>
        </div>
      </div>
    </div>
  `;
...
function toggleRecycleTarget() {
  const isNew = document.querySelector('input[name="recycle-target"]:checked').value === 'new';
  document.getElementById('recycle-existing').style.display = isNew ? 'none' : 'block';
  document.getElementById('recycle-new').style.display = isNew ? 'block' : 'none';
}
...
async function executeRecycle() {
  ...
  if (!isNew) {
    const posId = document.getElementById('recycle-pos').value;
    if (!posId) return alert('请选择持仓');
    body = { target_position_id: posId };
  } else {
    const sym = document.getElementById('recycle-symbol').value.trim().toUpperCase();
    const qty = parseFloat(document.getElementById('recycle-qty').value);
    const price = parseFloat(document.getElementById('recycle-price').value);
    if (!sym || !qty || !price) return alert('请填写完整信息');
    body = { new_symbol: sym, new_quantity: qty, new_price: price };
  }
  
  try {
    const res = await fetch('/api/profit-recycling/execute', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
