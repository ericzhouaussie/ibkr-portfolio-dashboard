...
def load_portfolio():
    if PORTFOLIO_FILE.exists():
        ...
        # 迁移：添加 profit_recycling 配置
        if "profit_recycling" not in data:
            data["profit_recycling"] = {
                "wheel_to_dca": {
                    "enabled": True,
                    "threshold": 5000,
                    "total_recycled": 0,
...
@app.route("/api/profit-recycling/status", methods=["GET"])
def get_profit_recycling_status():
    """获取 Wheel→DCA 盈利再投资状态"""
    portfolio = load_portfolio()
    recycling = portfolio.get("profit_recycling", {}).get("wheel_to_dca", {})
    if not recycling:
    ...
    wheel_pnl = 0
    for p in portfolio.get("positions", []):
        if p.get("strategy") == "wheel":
            wheel_pnl += p.get("pnl", 0)
    
    threshold = recycling.get("threshold", 5000)
    total_recycled = recycling.get("total_recycled", 0)
    
    # 已循环的利润（达到阈值的完整周期数 × 阈值）
    cycled_profit = int(total_recycled / threshold) * threshold if threshold > 0 else 0
    current_cycle_profit = wheel_pnl - cycled_profit
    progress = max(0, min(100, (current_cycle_profit / threshold * 100) if threshold > 0 else 0))
    can_recycle = current_cycle_profit >= threshold
    
    return jsonify({
        "enabled": recycling.get("enabled", True),
        "threshold": threshold,
        "total_recycled": total_recycled,
        "wheel_total_pnl": round(wheel_pnl, 2),
        "current_cycle_profit": round(current_cycle_profit, 2),
        "progress_pct": round(progress, 1),
        "can_recycle": can_recycle,
        "recycle_amount": threshold if can_recycle else 0,
        "history": recycling.get("history", [])[-10:],  # 最近10条
    })

@app.route("/api/profit-recycling/configure", methods=["POST"])
def configure_profit_recycling():
    """配置盈利再投资参数"""
    data = request.get_json() or {}
    portfolio = load_portfolio()
    
    if "profit_recycling" not in portfolio:
        portfolio["profit_recycling"] = {"wheel_to_dca": {}}
    
    rc = portfolio["profit_recycling"]["wheel_to_dca"]
    
    if "enabled" in data:
        rc["enabled"] = bool(data["enabled"])
    if "threshold" in data:
        rc["threshold"] = max(100, float(data["threshold"]))
    
    save_portfolio(portfolio)
    return jsonify({"success": True, "config": rc})

@app.route("/api/profit-recycling/execute", methods=["POST"])
def execute_profit_recycling():
    """执行盈利再投资：将 Wheel 盈利转入 DCA 持仓"""
    data = request.get_json() or {}
    portfolio = load_portfolio()
    
    recycling = portfolio.get("profit_recycling", {}).get("wheel_to_dca", {})
    if not recycling.get("enabled", True):
        return jsonify({"error": "盈利再投资未启用"}), 400
    
    threshold = recycling.get("threshold", 5000)
    
    # 计算 Wheel 总盈利和可支配再投资金额
    wheel_pnl = sum(p.get("pnl", 0) for p in portfolio.get("positions", []) if p.get("strategy") == "wheel")
    total_recycled = recycling.get("total_recycled", 0)
    cycled_profit = int(total_recycled / threshold) * threshold if threshold > 0 else 0
    current_cycle_profit = wheel_pnl - cycled_profit
    
    if current_cycle_profit < threshold:
        return jsonify({"error": f"当前周期盈利 ${current_cycle_profit:.2f} 未达到阈值 ${threshold:.2f}"}), 400
    
    recycle_amount = threshold  # 每次循环固定再投资 threshold
    
    # 目标 DCA 持仓（用户选择）
    target_pos_id = data.get("target_position_id", "")
    new_symbol = data.get("new_symbol", "").strip().upper()
    new_quantity = float(data.get("new_quantity", 0) or 0)
    new_price = float(data.get("new_price", 0) or 0)
    ...
    if target_pos_id:
        ...
        for p in portfolio["positions"]:
            if p["id"] == target_pos_id and p["strategy"] == "dca":
                target_pos = p
                break
        if not target_pos:
            return jsonify({"error": "目标 DCA 持仓不存在"}), 404
        
        # 计算买入数量（用再投资金额 / 当前价）
        current_price = target_pos.get("current_price") or target_pos.get("avg_price", 0)
        add_qty = int(recycle_amount / current_price) if current_price > 0 else 0
        if add_qty <= 0:
            return jsonify({"error": "计算买入数量失败，当前价格异常"}), 400
        
        # 更新持仓（平均成本法）
        old_qty = target_pos["quantity"]
        old_avg = target_pos["avg_price"]
        new_cost = recycle_amount
        target_pos["quantity"] = old_qty + add_qty
        target_pos["avg_price"] = round((old_qty * old_avg + new_cost) / (old_qty + add_qty), 4)
        target_pos["market_value"] = round(target_pos["quantity"] * (target_pos.get("current_price") or target_pos["avg_price"]), 2)
        
        action_desc = f"加仓 {add_qty}股，均价 ${target_pos['avg_price']:.2f}"
        
    elif new_symbol and new_quantity > 0 and new_price > 0:
        # 新建 DCA 持仓
        add_qty = new_quantity
        new_pos = {
            "id": generate_id("p"),
            "symbol": new_symbol,
            "strategy": "dca",
            "quantity": add_qty,
            "avg_price": round(new_price, 4),
            "current_price": new_price,
            "market_value": round(add_qty * new_price, 2),
            "pnl": 0,
            "pnl_pct": 0,
            "notes": f"盈利再投资 {now}",
        }
        portfolio["positions"].append(new_pos)
        action_desc = f"新建 {new_symbol} {add_qty}股 @ ${new_price:.2f}"
    else:
        return jsonify({"error": "请选择目标持仓或填写新建持仓信息"}), 400
    
    # 更新再投资记录
    recycling["total_recycled"] = total_recycled + recycle_amount
    recycling.setdefault("history", []).append({
        "date": now,
        "amount": recycle_amount,
        "target": target_pos_id or new_symbol,
        "action": action_desc,
    })
    ...
    portfolio.setdefault("history", []).append({
        ...
        "date": now,
        "action": "盈利再投资",
        "symbol": target_pos_id or new_symbol,
        "amount": recycle_amount,
        "note": action_desc,
    })
